...
function populateCartPage() {
  ...
  if (cartItems && cartItems.length > 0) {
    ...
    cartItems.forEach((item) => {
      ...
      const price = item.price * (item.count || 1); // Calculate the price of the item
    totalPrice += price; 
      details.innerHTML = `
        <h4>${item.title}</h4>
        <div class="count-container">
          <button class="count-btn decrement" data-item-id="${
            item.id
          }">-</button>
          <span class="count" id="count-${item.id}">${item.count || 1}</span>
          <button class="count-btn increment" data-item-id="${
            item.id
          }">+</button>
        </div>
...
function decrementCount(itemId) {
  ...
  cartItems = cartItems.map((item) => {
    if (parseInt(item.id) === parseInt(itemId) && (item.count || 1) > 0) {
      const count = (item.count || 1) - 1;
      const price = item.price * count; // Recalculate the price based on the updated count
